...
const createExperience = async (experienceData, path) => {
  ...
  let resultsDat
  if (path) {
    resultsExp = await connection.query("INSERT INTO experiences (experienceName, experienceDescription, price, photo, place_id) VALUES ( ?, ?, ?, ?, ? )  ", [experienceData.experienceName, experienceData.experienceDescription, experienceData.price, path, experienceData.place_id])
...
const editExperience = async (experienceData, experienceId, path) => {
  ...
  let resultsDat
  if (path) {
    resultsExp = await connection.query("UPDATE experiences SET experienceName = ?, experienceDescription = ?, price = ?, photo = ?, place_id = ? WHERE id = ? ", [experienceData.experienceName, experienceData.experienceDescription, experienceData.price, path, experienceData.place_id, experienceId])
    resultsDat = await connection.query("UPDATE dates SET totalSeats = ?, experienceDate = ?,  experienceHour = ? WHERE id = ?  ", [experienceData.totalSeats, experienceData.experienceDate, experienceData.experienceHour, experienceId])
  } else {
    resultsExp = await connection.query("UPDATE experiences SET experienceName = ?, experienceDescription = ?, price = ?,  place_id = ? WHERE id = ? ", [experienceData.experienceName, experienceData.experienceDescription, experienceData.price, experienceData.place_id, experienceId])
    resultsDat = await connection.query("UPDATE dates SET  totalSeats = ?, experienceDate = ?,  experienceHour = ? WHERE id = ?  ", [experienceData.totalSeats, experienceData.experienceDate, experienceData.experienceHour, experienceId])
  }
  return (resultsExp, resultsDat)
}
